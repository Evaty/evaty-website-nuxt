...
const { t } = useI18n()
const {data: category, pending} = await useFetch('https://api.evaty.net/api/v1/event/category/CLUB/?page=0')
const dayjs = useDayjs();

useHead({
  title: t("categorys.club.label"),
  bodyAttrs: {
    class: "act-category"
  }
})
...
<template>
  ...
  <section class="container category-header d-flex justify-content-center flex-column mb-100">
    <div class="category-header-headline pb-10">
      <h1 class="mb-0">{{ t("categorys.club.label") }}</h1>
    </div>
    <div class="category-header-description pb-20">
      {{t("categorys.club.description")}}
    </div>
    ...
    <img src="@/assets/images/categorys/header-images/club-header.png" alt=""/>
  </section>
